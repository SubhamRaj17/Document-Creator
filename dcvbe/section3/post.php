<?php
	...
	if($_POST['token'])
	{
		...
		$sql = "DELETE FROM section3_serverdetails
	    ...
	    while($ctr>0)
	    {
	    	if(isset($_POST['appName'.$ctr]))
	    	{
	    		...
	    		$Access = $_POST['Access'.$ctr];

	    		$sql = "INSERT INTO section3_serverdetails (sec_sign,system_type,app_role,app_name,version,ip,hostname,username,password,Access)
	    				VALUES ('$token','$type','$appRole','$appName','$version','$ip','$hostname','$username','$password','$Access'); " ;
	    		mysqli_query($dcConn,$sql);
	    	}
	    	$ctr--;
	    }
