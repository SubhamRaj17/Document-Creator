	...
	if(isset($_GET['prs']))
	{
		...
		if($resultCheck>0)
		{
			...
			if($resultCheck>0)
			{
				...
				//Query Integration Plans Table
				$sql = "SELECT *
						FROM  section8_integrationplan
				...
				{
					$customizations["site"] = $row[2] ;
					$customizations["description"] = $row[3] ;
					$customizations["details"] = $row[4] ;
					$customizations["solution"] = $row[5] ;
					$customizations["screenshot"] = $row[6] ;
					$customizations["remark"] = $row[7] ;
					array_push($response["customizations"], $customizations) ;
				}
