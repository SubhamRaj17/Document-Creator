...
$.getJSON(url,function(response)
{
	...
	{
		rowString = '<tr>	<td>'+techDetail.type+'</td>	<td>'+techDetail.appRole+'</td>	<td>'+techDetail.appName+'</td>	<td>'+techDetail.version+'</td>	<td>'+techDetail.ip+'</td>	<td>'+techDetail.hostname+'</td>	<td>'+techDetail.username+'</td>	<td>'+techDetail.password+'</td>	<td>'+techDetail.Access+'</td>	</tr>' ;
...
$.getJSON(url,function(response)
{
	...
	$.each(response.sampleMessages,function(key,sampleMessage)
	{
		...
		else if (sampleMessage.msg_type==4) {msgType = 'SIU';}
		var msgSource = '' ;
		if (sampleMessage.msg_source==1) {msgSource = 'Staging';}
		else if (sampleMessage.msg_source==2) {msgSource = 'Production';}
